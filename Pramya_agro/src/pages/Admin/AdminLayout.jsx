...
export default function AdminLayout() {
  ...
  const handleLogout = () => {
    logout(); // removes token, clears user state, navigates to login
  };
  ...
  const navItems = [
    ...
    { to: "/admin/careers", label: "Careers", icon: <Users size={20} /> }
  ];
  ...
  return (
    <div className="flex min-h-screen bg-[#020404] text-gray-200 font-sans">
      ...
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-[#0a0c0c] border-r border-white/5 transform transition-all duration-300 ease-in-out ${open ? "translate-x-0" : "-translate-x-full"} md:translate-x-0`}>
        <div className="flex flex-col h-full p-6">
          ...
          <nav className="flex-1 space-y-2">
            <p className="px-4 text-[10px] font-bold uppercase tracking-[0.2em] text-gray-500 mb-4">Main Menu</p>
            {navItems.map((item) => (
              <NavLink
                key={item.to}
                to={item.to}
          ...
          <div className="pt-6 border-t border-white/5 space-y-2">
            ...
            <button 
              onClick={handleLogout} // <-- now calls centralized logout
      ...
      <div className="flex-1 flex flex-col min-w-0 md:ml-72">
        <header className="md:hidden flex items-center justify-between p-4 bg-[#0a0c0c] border-b border-white/5">
          ...
          <button onClick={() => setOpen(!open)} className="p-2 bg-emerald-500/10 text-emerald-500 rounded-lg">
